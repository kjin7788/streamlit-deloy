...
# precision(정밀도): 모델이 '비만' 이라고 예측한 것 중 실제로 맞는 비율 => '과체중'을 예측 했을때 실제로 과체중인 비율
# Recall(재현율): 실제'비만'중에서 모델이 맞춘 비율 => '비만'인 사람을 놓치지 않고 얼마나 예측했는가 
# F1-score : Precision과 Recall의 조화평균 => 모델의 적체적인 성능을 나타냄
...
# 홈화면
def home():
    st.title("Obesity Classifiaction DashBoard")
    st.markdown("""
    - **Age**: 나이
